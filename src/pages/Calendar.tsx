...
const CalendarPage = () => {
  ...
  const getDayHasCompletions = (day: Date) => {
    ...
    const dayCompletions = completions.filter(c => format(new Date(c.date), 'yyyy-MM-dd') === dateStr);
        ...
        <CardContent>
          <div className="flex justify-center">
            <Calendar
              ...
              modifiersClassNames={{
                completed: 'bg-habit-success text-white hover:bg-habit-success hover:text-white',
                partial: 'bg-habit-warning text-white hover:bg-habit-warning hover:text-white',
              }}
            />
          </div>
          <div className="mt-4 flex justify-center gap-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">{t('allHabits', 'calendar')}</span>
            </div>
            <div className="flex items-center gap-2">
              <div className="h-3 w-3 rounded-full bg-habit-warning" />
              <span className="text-sm text-muted-foreground">{t('someHabits', 'calendar')}</span>
            </div>
          </div>
        </CardContent>
      </Card>
