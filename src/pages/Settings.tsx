...
export default function Settings() {
  const [notifications, setNotifications] = useState(
    getInitialSettings().notifications
  )
  ...
  // Apply theme from localStorage on component mount, only if needed
  useEffect(() => {
    if (theme === "dark") {
  ...
  const handleThemeChange = (isDark: boolean) => {
    ...
    if (theme !== newTheme) {
      ...
      toast({
        ...
        description: t("themeChangedDesc", "settings").replace(
          "{theme}",
          themeValue
        ),
      })
  ...
  const handleLanguageChange = (value: string) => {
    ...
    toast({
      ...
      description: t("languageChangedDesc", "settings").replace(
        "{language}",
        languageName
      ),
    })
  ...
  const handleNotificationsChange = (enabled: boolean) => {
    ...
    toast({
      title: t(
        enabled ? "notificationsEnabled" : "notificationsDisabled",
        "settings"
      ),
      description: t(
        enabled ? "notificationsEnabledDesc" : "notificationsDisabledDesc",
        "settings"
      ),
    })
  }
        ...
        <CardContent className="space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <div className="text-sm text-muted-foreground">
                {theme === "light"
                  ? t("lightTheme", "settings")
                  : t("darkTheme", "settings")}
              </div>
            </div>
            <Switch
              checked={theme === "dark"}
